...
def solution(board, moves):
    ...
    cnt = 0
    n = len(board)
    new_board = [[board[(n-1)-j][i] for j in range(n) if board[(n-1)-j][i]] for i in range(n)]       # 열 별로 정렬된 borad로 새로 변경
    for move in moves:
        if new_board[move-1]:            
            target = new_board[move-1].pop()
            if len(basket) and basket[-1] == target:
                basket.pop()
                cnt +=2
            else :
