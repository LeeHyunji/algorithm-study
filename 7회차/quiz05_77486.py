...
def solution(enroll, referral, seller, amount):
    price = 100                                                                             # 칫솔 개당 가격 100원
    result = [0]*(len(enroll)+1)                                                            # 이익금 초기화 (center 포함)
    
    people = {enroll[i]: i for i in range(len(enroll))}                                     # **10번 시간초과 해결 -> 딕셔너리를 통해서 index() 대체

    for i in range(len(seller)):
        seller_index = people[seller[i]]                                                    # 판매자 정보
        total_price = amount[i]*price
        while True:
            referral_name = referral[seller_index]                                          # 윗단계 추천인 정보
            referral_index = -1 if referral_name == "-" else people[referral_name]          # 추천인이 "-"일경우 cetner를 의미

            referral_price = int(total_price*0.1)                                           # 추천인 이익금 : 이익금액에 10%(절사)
            seller_price = total_price-referral_price                                       # 본인 이익금 : 이익금액 - 추천인 이익금
            result[seller_index] += seller_price                                                

            if referral_index == -1 or not(referral_price):                                 # **11번 12번 시간초과 해결 -> 추천인 이익금이 0일 경우 break 처리
                break
            total_price = referral_price
            seller_index = referral_index
