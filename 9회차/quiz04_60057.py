...
def solution(s):
    if(len(s)==1):                                                    # 테스트5 런타임에러) S의 길이가 1일때 조건 
        return 1
    
    result = []
    for l in range(1,len(s)+1//2):                                    # 압축을 할려면 적어도 2번이상은 반복해야되므로 압축 단위는 문자열의 절반 크기까지
        cnt=1                                                         # 문자열 압축 첫번째 초기화
        zip_s=""
        pattern= s[0:l]
        for i in range(l,len(s)+1,l):                                 # 두번재(l)부터 검사
            string = s[i:i+l]                                         
            if pattern==string:
                cnt+=1
            else:
                zip_s += str(cnt) if cnt>1 else ""
                zip_s += pattern
                cnt=1
                pattern = s[i:i+l]
        zip_s += s[:len(s)%l]                                         # 마지막에 압축 단위 길이만큼 남지 않았을때 처리
        result.append(len(zip_s))
